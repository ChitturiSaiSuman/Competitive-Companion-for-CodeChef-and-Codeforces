...
def run_java(program_to_be_executed: str, online_judge: bool, stdin: str, expected: str) -> None:
    ...
    if compilation_log:
        ...
        if online_judge:
            ...
            start_time = time()
            execution_log = run(['timeout', '-t', str(java_time_limit), '-m', memory_string, '-x', speed_string, 'java', 'Main'], capture_output = True, text = True, input = stdin, check = True)
            end_time = time()
            ...
            speed_string = str(max_boost_speed)
            start_time = time()
            execution_log = run(['timeout', '-m', memory_string, '-x', speed_string, 'java', 'Main'], capture_output = True, text = True, input = stdin, check = True)
            end_time = time()
            stdout = execution_log.stdout.strip()
            stderr = execution_log.stderr.strip()
            stderr = '\n'.join(list(map(str, stderr.split('\n')))[:-1])
...
def run_cpp(program_to_be_executed: str, online_judge: bool, stdin: str, expected: str) -> None:
    ...
    if compilation_log:
        ...
        if online_judge:
            ...
            speed_string = str(max_boost_speed)
            start_time = time()
            execution_log = run(['timeout', '-t', str(cpp_time_limit), '-m', memory_string, '-x', speed_string, './exe'], capture_output = True, text = True, input = stdin, check = True)
            end_time = time()
            stdout = execution_log.stdout.strip()
            stderr = execution_log.stderr.strip()
            stderr = '\n'.join(list(map(str, stderr.split('\n')))[:-1])
            ...
            speed_string = str(max_boost_speed)
            start_time = time()
            execution_log = run(['timeout', '-m', memory_string, '-x', speed_string, './exe'], capture_output = True, text = True, input = stdin, check = True)
            end_time = time()
            stdout = execution_log.stdout.strip()
            stderr = execution_log.stderr.strip()
            stderr = '\n'.join(list(map(str, stderr.split('\n')))[:-1])
...
def run_c(program_to_be_executed: str, online_judge: bool, stdin: str, expected: str) -> None:
    ...
    if compilation_log:
        ...
        if online_judge:
            ...
            speed_string = str(max_boost_speed)
            start_time = time()
            execution_log = run(['timeout', '-t', str(c_time_limit), '-m', memory_string, '-x', speed_string, './exe'], capture_output = True, text = True, input = stdin, check = True)
            end_time = time()
            stdout = execution_log.stdout.strip()
            stderr = execution_log.stderr.strip()
            stderr = '\n'.join(list(map(str, stderr.split('\n')))[:-1])
...
def run_python(program_to_be_executed: str, online_judge: bool, stdin: str, expected: str) -> None:
    ...
    if online_judge:
        ...
        speed_string = str(max_boost_speed)
        start_time = time()
        execution_log = run(['timeout', '-t', str(python_time_limit), '-m', memory_string, '-x', speed_string, 'python3', program_to_be_executed], capture_output = True, text = True, input = stdin, check = True)
        end_time = time()
        stdout = execution_log.stdout.strip()
        stderr = execution_log.stderr.strip()
        stderr = '\n'.join(list(map(str, stderr.split('\n')))[:-1])
        ...
        speed_string = str(max_boost_speed)
        start_time = time()
        execution_log = run(['timeout', '-m', memory_string, '-x', speed_string, 'python3', program_to_be_executed], capture_output = True, text = True, input = stdin, check = True)
        end_time = time()
        stdout = execution_log.stdout.strip()
        stderr = execution_log.stderr.strip()
        stderr = '\n'.join(list(map(str, stderr.split('\n')))[:-1])
